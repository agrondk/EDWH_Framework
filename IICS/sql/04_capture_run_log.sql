...
-- Purpose : Read the latest entity-level run entries from
--           ETL_RUN_LOG for today's run of both entities.
--           Populates taskflow variables used in the
--           email notifications.
--
...
--   STAFFING_ROWS_REJECTED → $$v_staffing_rows_rejected
--   COST_ROWS_INSERTED     → $$v_cost_rows_inserted
--   COST_ROWS_REJECTED     → $$v_cost_rows_rejected
--
...
SELECT
    ...
    NVL(MAX(CASE WHEN ENTITY_NAME = 'STAFFING_SCHEDULE' THEN ROWS_REJECTED END), 0) AS STAFFING_ROWS_REJECTED,

    NVL(MAX(CASE WHEN ENTITY_NAME = 'COST' THEN ROWS_INSERTED END), 0)      AS COST_ROWS_INSERTED,
    NVL(MAX(CASE WHEN ENTITY_NAME = 'COST' THEN ROWS_REJECTED END), 0)      AS COST_ROWS_REJECTED

FROM
    GPC_DM.V_IICS_RUN_SUMMARY
WHERE
    TRUNC(START_TIME) = TRUNC(SYSDATE)
    AND MAPPING_ID IS NULL   -- entity-level entries only (not per-mapping child rows)
